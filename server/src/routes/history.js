...
router.get('/', async (req, res, next) => {
  try {
    ...
    const completionsResult = await db.execute({
      sql: 'SELECT date, day FROM day_completions WHERE date >= ? ORDER BY date ASC',
      args: [startStr],
    });
    const cardioResult = await db.execute({
      sql: 'SELECT date FROM cardio_completions WHERE date >= ? ORDER BY date ASC',
      args: [startStr],
    });

    const completedByDate = Object.fromEntries(
      completionsResult.rows.map((r) => [r.date, r.day])
    );
    const cardioSet = new Set(cardioResult.rows.map((r) => r.date));
    ...
    res.json({ from: startStr, to: todayStr(), entries: result, streak: await computeStreak() });
  } catch (err) {
    next(err);
  }
});
